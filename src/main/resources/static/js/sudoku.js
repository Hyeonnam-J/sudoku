...
function getSudokuData() {
    fetch(`/sudoku/data?difficulty=${selectedDifficulty}`)
        // response 이 응답 개체에는 HTTP 응답과 관련된 헤더 및 기타 정보가 포함.
        // response 객체에서 .json()을 호출하면 응답 본문을 JSON으로 구문 분석하고 JavaScript 객체(파싱된 JSON)로 확인되는 Promise를 반환.
        // 결국 data, gData는 json이 아닌 서버에서 반환하는 타입 SudokuData.
